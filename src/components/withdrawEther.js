...
export default class WithdrawEther extends Component{
    async componentDidMount(dispatch) {
        ...
        try {
            ...
            if(element == "Ether"){
                this.setState({Ether: this.state.userSupply['LendAmount'][index] / 10 **18})
            }
            }
        } catch (e) {
                console.log(e)
    ...
    async withdrawEther(){
        try {
            await this.state.iuhBank.methods.withdrawEther((this.state.amount * 10 ** 18).toString()).send({from: this.state.account});
            this.handleNextTab();
        } catch (error) {
            console.log(error)
        }
                ...
                <div>
                        <div className="deposit">
                            ...
                            <div className="text-center mt-5">
                                ...
                                <form>
                                    <Form.Label htmlFor="disabledTextInput">Avalable to withdraw:  {this.state.Ether.toLocaleString()} Ether</Form.Label>
