...
export default class RepayIuhCoin extends Component{
    async componentDidMount(dispatch) {
        ...
        try {
            ...
            let getIuhCoinPoolInfo = await IuhBank.methods.getIuhCoinPoolInfo().call({from: this.state.account});
            for (let index = 0; index < userBorrow['tokenName'].length; index++) {
            const element =  userBorrow['tokenName'][index];
            if(element == "IuhCoin"){
                this.setState({Repay: (userBorrow['Borrow'][index]/ getIuhCoinPoolInfo['price']) / 10 **18})
            }
            }
                    ...
                    <div>
                        <div className="deposit">
                            ...
                            <div className="text-center mt-5">
                                ...
                                <form>
                                    <Form.Label htmlFor="disabledTextInput">Amount to Repay: {this.state.Repay != null && this.state.Repay.toLocaleString()} IuhCoin</Form.Label>
                                </form>
