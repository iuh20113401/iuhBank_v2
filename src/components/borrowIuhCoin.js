...
export default class BorroIuhCoin extends Component{
    async componentDidMount(dispatch) {
        ...
        try {
            ...
            let iuhCoinPool = await IuhBank.methods.getIuhCoinPoolInfo().call({from: this.state.account});
            let Avaliable = (UserSupply['Collateral'] / iuhCoinPool['price'])/10**18 ;
            this.setState({token: iuhCoin, iuhBank: IuhBank, iuhBankAddress: iuhBankAddress,Avaliable: Avaliable})
        } catch (e) {
                console.log(e)
    ...
    async borrowAmount(amount){
        this.setState({amount: amount, connect: true});
    }
    async borrowIuhCoin(){
        try {
            await this.state.iuhBank.methods.borrowingIuhCoin(this.state.amount).send({from: this.state.account});
            this.handleNextTab();
        } catch (error) {
            console.log(error)
        }
                ...
                <div>
                        <div className="deposit">
                            ...
                            <div className="text-center mt-5">
                                ...
                                <p>
                                    Please enter an amount you would like to borrow. The maxium amount you <br></br> can borrow is shown below
                                </p>
                                <form>
                                    <Form.Label htmlFor="disabledTextInput">Avaliable to borrow: {this.state.Avaliable != null && (this.state.Avaliable).toLocaleString()} IuhCoin</Form.Label>
                                    ...
                                    onClick={(e) => {
                                            e.preventDefault()
                                            let amount = this.borrow.value;
                                            if(amount> (this.state.Avaliable)){
                                                alert("Bạn không đủ thế chấp để mượn tiền");
                                            }else{
                    ...
                    <div>
                        <div className="deposit">
                            ...
                            <div className="text-center mt-5">
                                ...
                                <form>
                                    <Form.Label htmlFor="disabledTextInput">Amount to borrow {this.state.amount} IuhCoin</Form.Label>
                                </form>
                                <div className="mb-3 w-50 m-auto ">
                                    <Tabs activeKey={this.state.activeTab} onSelect={this.handleSelect} variant="pills" justify className = 'bg-light w-100 m-auto border'>
                                        <Tab eventKey={1} title="1 Borrow" disabled>
                                            <Tab.Content className="row">
                                                <h6 className="p-3 col-9" style={{textAlign: "start"}}><span className="h5 text-primary">1/2 Borrow</span> <br/>Pleasde submit to borrow</h6>
                                                <Button onClick={(e) => {
                                                    e.preventDefault()
                                                    this.borrowIuhCoin();
                                                    }} 
                                                variant="btn btn-primary h-25 m-auto">Borrow</Button>
                                            </Tab.Content>
                                        </Tab>
